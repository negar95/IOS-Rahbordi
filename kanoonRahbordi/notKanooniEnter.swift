...
class notKanooniEnter: UIViewController , UIPickerViewDataSource, UIPickerViewDelegate, NVActivityIndicatorViewable, InternetStatusIndicable{
    ...
    override func viewDidLoad() {
        ...
        userPhoneNum.addTarget(self, action: #selector(textFieldDidChange(_:)), for: .editingDidBegin)
    ...
    @IBAction func notKanooniEnterButton(_ sender: UIButton) {
        ...
        if phones.characters.count == 11 {
            self.startAnimating(self.size, message: "در حال احراز هویت...", type: .triangleSkewSpin, color: self.rColor, textColor: .white)
            ...
            warning.text = "لطفا اطلاعات را صحیح وارد کنید"
        }
