...
class QuizViewController: UIViewController, NVActivityIndicatorViewable, InternetStatusIndicable {
    ...
    override func viewDidLoad() {
        ...
        self.startAnimating(size, message: "بارگیری آزمون...", type: .triangleSkewSpin, color: .orange, textColor: .white)
    ...
    @IBAction func nextQuestion(_ sender: Any) {
        ...
        if self.quizNum<9 {
            self.quizNum += 1
            self.startAnimating(size, type: .triangleSkewSpin, color: .orange, textColor: .white)
            self.setImg(imgUrl: self.quizArr[self.quizNum].QuestionImageAddress, img: self.questionImg)
            self.stopAnimating()
            self.warning.text = "سوال \(self.quizNum+1)"
            self.answerCheck()
